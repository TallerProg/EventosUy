...
public class ConsultaEdicionEvento extends JInternalFrame {
    ...
    private JTextField nombreEdicion, ciudadEdicion, siglaEdicion, paisEdicion, fAltaEdicion, fInicioEdicion,
            fFinEdicion;
    // NUEVO: campo para mostrar el estado de la edición (solo lectura)
    private JTextField estadoEdicion; // NUEVO
    ...
    public ConsultaEdicionEvento(IControllerEvento ice) {
        ...
        this.controller = ice;
        setSize(530, 480); // NUEVO: un poco más de alto para el campo Estado
    ...
    private void initComponents() {
        ...
        // NUEVO: fila de Estado
        GridBagConstraints gbcLblEstado = new GridBagConstraints();
        gbcLblEstado.gridx = 0;
        gbcLblEstado.gridy = row;
        gbcLblEstado.anchor = GridBagConstraints.WEST;
        gbcLblEstado.insets = new Insets(5, 5, 5, 5);
        panel.add(new JLabel("Estado :"), gbcLblEstado);

        estadoEdicion = new JTextField(); // NUEVO
        estadoEdicion.setEditable(false); // NUEVO
        GridBagConstraints gbcEstado = new GridBagConstraints();
        gbcEstado.gridx = 1;
        gbcEstado.gridy = row;
        gbcEstado.gridwidth = 3;
        gbcEstado.fill = GridBagConstraints.HORIZONTAL;
        gbcEstado.insets = new Insets(5, 5, 5, 5);
        panel.add(estadoEdicion, gbcEstado);
        row++;
        // FIN NUEVO
    ...
    private void mostrarDatosEdicion(DTEdicion dtEd) {
        ...
        // NUEVO: mostrar estado (enum o string; si es null queda vacío)
        try {
            Object estado = dtEd.getEstado(); // si tu método se llama distinto, cambialo acá
            estadoEdicion.setText(estado == null ? "" : String.valueOf(estado));
        } catch (Exception ignore) {
            estadoEdicion.setText("");
        }
        // FIN NUEVO
