...
public class ModificarUsuario extends JInternalFrame {
    ...
    public ModificarUsuario(IControllerUsuario icu, IControllerInstitucion ici) {
        ...
        comboUsuarios.setRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
                ...
                if (value instanceof Usuario usu) {
                    setText(usu.getNickname());
                }
                return this;
            }
        });
    ...
    private void actualizarCampos() {
        Usuario usu = (Usuario) comboUsuarios.getSelectedItem();
        if (usu == null) return;
        ...
        textFieldNickName.setText(usu.getNickname());
        textFieldCorreo.setText(usu.getCorreo());
        textFieldNombre.setText(usu.getNombre());

        if (usu instanceof Asistente asi) {
            comboTipoUsuario.setSelectedItem("Asistente");
            lblApellido.setVisible(true);
            textFieldApellido.setVisible(true);
            textFieldApellido.setText(asi.getApellido());

            lblFecha.setVisible(true);
            textFieldFechaNacimiento.setVisible(true);
            textFieldFechaNacimiento.setText(asi.getfNacimiento().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
            ...
            comboInstitucion.setSelectedItem(asi.getInstitucion() != null ? asi.getInstitucion().getNombre() : "Ninguna");

        } else if (usu instanceof Organizador org) {
            comboTipoUsuario.setSelectedItem("Organizador");
            lblDescripcion.setVisible(true);
            textFieldDescripcion.setVisible(true);
            textFieldDescripcion.setText(org.getDescripcion());

            lblURL.setVisible(true);
            textFieldURL.setVisible(true);
            textFieldURL.setText(org.getUrl() != null ? org.getUrl() : "");
        }
    }
    ...
    private void modificarUsuario() {
        ...
        try {
            usu.setCorreo(textFieldCorreo.getText().trim());
            usu.setNombre(textFieldNombre.getText().trim());

            if (usu instanceof Asistente asi) {
                asi.setApellido(textFieldApellido.getText().trim());
                LocalDate fecha = LocalDate.parse(textFieldFechaNacimiento.getText().trim(),
                        DateTimeFormatter.ofPattern("dd/MM/yyyy"));
                asi.setfNacimiento(fecha);
                String instName = (String) comboInstitucion.getSelectedItem();
                asi.setInstitucion(instName.equals("Ninguna") ? null : controlIns.findInstitucion(instName));

            } else if (usu instanceof Organizador org) {
                org.setDescripcion(textFieldDescripcion.getText().trim());
                String url = textFieldURL.getText().trim();
                org.setUrl(url.isEmpty() ? null : url);
            }

            controlUsr.modificarUsuario1(usu);
