...
public class ConsultaPatrocinio extends JInternalFrame {
	...
	public static ConsultaPatrocinio crearYMostrar(IControllerEvento controller, DTPatrocinio dtPat,
		...
		ConsultaPatrocinio cpa = new ConsultaPatrocinio(controller);
		cpa.cargarDatos(dtPat);
		if (desktopPane != null) {
			desktopPane.add(cpa);
			cpa.setVisible(true);
			cpa.setLocation((desktopPane.getWidth() - cpa.getWidth()) / 2,
					(desktopPane.getHeight() - cpa.getHeight()) / 2);
		}
		return cpa;
	}
